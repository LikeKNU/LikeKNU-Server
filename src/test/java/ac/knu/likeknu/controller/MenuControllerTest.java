...
public class MenuControllerTest {
    ...
    void getMenuResponsesAndSuccess() throws Exception {
        ...
        List<MenuResponse> menuResponseList = List.of(menuResponse1, menuResponse2, menuResponse3);

        //when
        when(menuService.getMenuResponsesByCampus(Campus.CHEONAN))
                .thenReturn(menuResponseList);
        ...
        resultActions.andExpectAll(
                ...
                jsonPath("$.data.body.[2].meal.[0].mealType").value(menuResponse3.getMeal().get(0).getMealType()),
                jsonPath("$.data.body.[2].meal.[1].operatingTime").value(menuResponse3.getMeal().get(1).getOperatingTime()),
                jsonPath("$.data.body.[1].meal.[2].menus").value(menuResponse2.getMeal().get(2).getMenus()),
                jsonPath("$.data.body.[2].meal.[2].menus.[1].menuId").value(menuResponse3.getMeal().get(2).getMenus().get(1).getMenuId()),
                jsonPath("$.data.body.[2].meal.[2].menus.[2].menuName").value(menuResponse3.getMeal().get(2).getMenus().get(2).getMenuName())
        ).andDo(print());
    }
}
